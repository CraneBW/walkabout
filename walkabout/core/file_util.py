...
def cached(url: str, prefix: str) -> str:
    ...
    ensure_directory_exists(os.path.join(os.environ.get("WALKABOUT_HOME", os.path.expanduser("~/.walkabout")), "files"))
    path = os.path.join(os.path.join(os.environ.get("WALKABOUT_HOME", os.path.expanduser("~/.walkabout")), "files"), prefix + "-" + url_hash + "-" + name)
